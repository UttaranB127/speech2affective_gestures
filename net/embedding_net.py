...
def conv_norm_relu(in_channels, out_channels, down_sample=False, padding=0, batch_norm=True):
    if not down_sample:
        k = 3
        s = 1
    else:
...
class PoseEncoderConv(nn.Module):
    def __init__(self, length, dim):
        ...
        self.net = nn.Sequential(
            conv_norm_relu(dim, 32, batch_norm=True),
            conv_norm_relu(32, 64, batch_norm=True),
            conv_norm_relu(64, 64, True, batch_norm=True),
            nn.Conv1d(64, 32, 3)
        )
        ...
        self.fc_log_var = nn.Linear(32, 32)
...
class ContextEncoder(nn.Module):
    ...
    def forward(self, in_text, in_spec):
        ...
        var_input = torch.cat((audio_feat_seq, text_feat_seq), dim=2)
        var_output, _ = self.gru(var_input)

        last_output = var_output[:, -1]
...
class EmbeddingNet(nn.Module):
    ...
    def forward(self, in_text, in_audio, pre_poses, poses, input_mode=None, variational_encoding=False):
        ...
        if self.context_encoder is not None and in_text is not None and in_audio is not None:
            context_feat, context_mu, context_log_var = self.context_encoder(in_text, in_audio)
            # context_feat = F.normalize(context_feat, p=2, dim=1)
        else:
            context_feat = context_mu = context_log_var = None

        # poses
        if poses is not None:
            poses_feat, pose_mu, pose_log_var = self.pose_encoder(poses, variational_encoding)
            # poses_feat = F.normalize(poses_feat, p=2, dim=1)
        else:
            poses_feat = pose_mu = pose_log_var = None
        ...
        return context_feat, context_mu, context_log_var, poses_feat, pose_mu, pose_log_var, out_poses
