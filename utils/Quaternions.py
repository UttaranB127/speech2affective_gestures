...
class Quaternions:
    ...
    def between(cls, v0s, v1s):
        ...
        no_rot_idx = np.argwhere(a_norms < 1e-6)[..., 0]
        a = a / a_norms[..., None]
        t = np.arccos(np.einsum('...i,...i->...',
                                v0s / np.linalg.norm(v0s, axis=-1)[..., None],
                                v1s / np.linalg.norm(v1s, axis=-1)[..., None]))
        q = Quaternions.from_angle_axis(t, a)
        if len(no_rot_idx) > 0:
