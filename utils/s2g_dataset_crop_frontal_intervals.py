...
parser.add_argument('-speaker', '--speaker', default=None,
                    help='optionally, run only on specific speaker', required=False)
...
if __name__ == '__main__':
    ...
    for _, interval in tqdm(df_intervals.iterrows(), total=len(df_intervals)):
        try:
            out_path = os.path.join(args.base_path, interval['speaker'], 'intervals')
            ...
            out_file = os.path.join(out_path, '{}_{}-{}.mp4'.format(interval['video_fn'],
                                                                    str(start_time), str(end_time)))
            print('Video: {}\nInterval: {}'.format(in_file, out_file))
