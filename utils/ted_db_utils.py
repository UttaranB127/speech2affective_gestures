...
def extract_mel_spectrogram(y, sr=16000):
    mel_spec = librosa.feature.melspectrogram(y=y, sr=sr, n_fft=1024, hop_length=512, power=2)
    log_mel_spec = librosa.power_to_db(mel_spec, ref=np.max)  # mels x time
    log_mel_spec = log_mel_spec.astype('float16')
    return log_mel_spec
